...
def get_forecast_stats(stats, rperiods, comid, records, sim):
    ...
    # Create the logo
    titulo = "Pronóstico de caudales del Rio Cuenca (Pte. Europa)"
    layout = go.Layout(
        title=titulo,
        yaxis={'title': 'Caudal (m3/s)', 'range': [min_visible, max_visible]},
...
# Read Paute data
est = pd.read_csv("puente-europa.dat", sep="\t", header=0)
